...
def run_sweep(args):
    ...
    if args.indices:
        sample_indices = np.array([int(x) for x in args.indices.split(',')])
        n_samples = len(sample_indices)
        print(f"Sweeping specific indices: {sample_indices}")
    else:
        ...
        for idx in tqdm(sample_indices, desc=f"Sweeping {model_name}"):
            ...
            try:
                ...
                if cf is not None:
                    ...
                    # Save Latent Plot
                    latent_bname = f"latent_win{idx}.png"
                    visualize_latent_space(
                        explainer, bg_pool, window, cf,
                        os.path.join(model_out_dir, latent_bname)
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--indices", type=str, default=None, help="Comma-separated list of specific window indices to run (overrides random sampling)")
